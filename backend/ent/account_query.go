...
func (_q *AccountQuery) QueryAllowedUsers() *UserQuery {
...
func (_q *AccountQuery) QueryAccountAllowedUsers() *AccountAllowedUserQuery {
...
func (_q *AccountQuery) WithAllowedUsers(opts ...func(*UserQuery)) *AccountQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) WithAccountAllowedUsers(opts ...func(*AccountAllowedUserQuery)) *AccountQuery {
	query := (&AccountAllowedUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withAllowedUsers; query != nil {
		if err := _q.loadAllowedUsers(ctx, query, nodes,
			func(n *Account) { n.Edges.AllowedUsers = []*User{} },
			func(n *Account, e *User) { n.Edges.AllowedUsers = append(n.Edges.AllowedUsers, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withAccountAllowedUsers; query != nil {
		if err := _q.loadAccountAllowedUsers(ctx, query, nodes,
			func(n *Account) { n.Edges.AccountAllowedUsers = []*AccountAllowedUser{} },
			func(n *Account, e *AccountAllowedUser) {
...
func (_q *AccountQuery) loadAllowedUsers(ctx context.Context, query *UserQuery, nodes []*Account, init func(*Account), assign func(*Account, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Account)
	nids := make(map[int64]map[*Account]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(account.AllowedUsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(account.AllowedUsersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(account.AllowedUsersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(account.AllowedUsersPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *AccountQuery) loadAccountAllowedUsers(ctx context.Context, query *AccountAllowedUserQuery, nodes []*Account, init func(*Account), assign func(*Account, *AccountAllowedUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(accountalloweduser.FieldAccountID)
	}
	query.Where(predicate.AccountAllowedUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.AccountAllowedUsersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
