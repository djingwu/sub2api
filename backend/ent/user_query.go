...
type UserQuery struct {
	...
	withWhiteListedAccounts   *AccountQuery
	...
	withAccountAllowedUsers   *AccountAllowedUserQuery
...
// QueryWhiteListedAccounts chains the current query on the "white_listed_accounts" edge.
func (_q *UserQuery) QueryWhiteListedAccounts() *AccountQuery {
...
// QueryAccountAllowedUsers chains the current query on the "account_allowed_users" edge.
func (_q *UserQuery) QueryAccountAllowedUsers() *AccountAllowedUserQuery {
	query := (&AccountAllowedUserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withWhiteListedAccounts:   _q.withWhiteListedAccounts.Clone(),
		...
		withAccountAllowedUsers:   _q.withAccountAllowedUsers.Clone(),
...
// WithWhiteListedAccounts tells the query-builder to eager-load the nodes that are connected to
// the "white_listed_accounts" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithWhiteListedAccounts(opts ...func(*AccountQuery)) *UserQuery {
	query := (&AccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withWhiteListedAccounts = query
	return _q
}
...
// WithAccountAllowedUsers tells the query-builder to eager-load the nodes that are connected to
// the "account_allowed_users" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAccountAllowedUsers(opts ...func(*AccountAllowedUserQuery)) *UserQuery {
	query := (&AccountAllowedUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAccountAllowedUsers = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [16]bool{
			...
			_q.withWhiteListedAccounts != nil,
			...
			_q.withAccountAllowedUsers != nil,
		}
	)
	...
	if query := _q.withWhiteListedAccounts; query != nil {
		if err := _q.loadWhiteListedAccounts(ctx, query, nodes,
			func(n *User) { n.Edges.WhiteListedAccounts = []*Account{} },
			func(n *User, e *Account) { n.Edges.WhiteListedAccounts = append(n.Edges.WhiteListedAccounts, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withAccountAllowedUsers; query != nil {
		if err := _q.loadAccountAllowedUsers(ctx, query, nodes,
			func(n *User) { n.Edges.AccountAllowedUsers = []*AccountAllowedUser{} },
			func(n *User, e *AccountAllowedUser) {
				n.Edges.AccountAllowedUsers = append(n.Edges.AccountAllowedUsers, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadWhiteListedAccounts(ctx context.Context, query *AccountQuery, nodes []*User, init func(*User), assign func(*User, *Account)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*User)
	nids := make(map[int64]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.WhiteListedAccountsTable)
		s.Join(joinT).On(s.C(account.FieldID), joinT.C(user.WhiteListedAccountsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.WhiteListedAccountsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.WhiteListedAccountsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "white_listed_accounts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *UserQuery) loadAccountAllowedUsers(ctx context.Context, query *AccountAllowedUserQuery, nodes []*User, init func(*User), assign func(*User, *AccountAllowedUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(accountalloweduser.FieldUserID)
	}
	query.Where(predicate.AccountAllowedUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AccountAllowedUsersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
