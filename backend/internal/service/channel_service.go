...
// cachePubSub 用于跨实例广播渠道缓存失效；可传 nil（此时仅本地失效，测试场景）。
// accountRepo 仅供模型广场读取账号支持的模型（model_mapping）；可传 nil（此时广场
// 回退为按渠道模型清单展示）。
func NewChannelService(repo ChannelRepository, groupRepo GroupRepository, authCacheInvalidator APIKeyAuthCacheInvalidator, pricingService *PricingService, cachePubSub ChannelCachePubSub, accountRepo AccountRepository) *ChannelService {
	s := &ChannelService{
		repo:                 repo,
		groupRepo:            groupRepo,
